...
CoapEndpoint::CoapEndpoint(std::string rootUri) {
  ...
  if (rootUri.find_last_of('/') == std::string::npos ||
      rootUri.find_last_of('/') != rootUri.length() - 1)
    rootUri += "/";

  localRootUri = coap_new_uri(
      reinterpret_cast<const uint8_t *>(rootUri.c_str()), rootUri.length());

  std::vector<char> uriBuf(localRootUri->host.length);
  strncpy(uriBuf.data(), reinterpret_cast<const char *>(localRootUri->host.s),
          localRootUri->host.length);
...
void CoapEndpoint::addResourceHandler(std::string const &path,
                                      coap_request_t const &method,
                                      coap_method_handler_t const &h) {
  coap_str_const_t *uri = coap_new_str_const(
      reinterpret_cast<const uint8_t *>(path.c_str()), path.length());
  if (uri == NULL)
    throw "Could not transform URI to a CoAP string";

  coap_resource_t *resource =
      coap_resource_init(uri, COAP_RESOURCE_FLAGS_RELEASE_URI);
  if (resource == NULL)
    throw "Could not create a resource for that URI";
  coap_add_resource(context, resource);

  coap_register_handler(resource, method, h);
  registeredResources.insert(
      std::pair<std::pair<std::string, coap_request_t>, coap_resource_t *>(
          std::pair<std::string, coap_request_t>(path, method), resource));
}

void CoapEndpoint::registerResponseHandler(
    coap_response_handler_t const &h) const {
...
void CoapEndpoint::unregisterResourceHandler(std::string const &path,
                                             coap_request_t const &method) {
  coap_resource_t *r = registeredResources.at(
      std::pair<std::string, coap_request_t>(path, method));
  coap_register_handler(r, method, nullptr);
  coap_delete_resource(context, r);
  registeredResources.erase(
      std::pair<std::string, coap_request_t>(path, method));
}
...
void CoapEndpoint::send(Message m) {
  ...
  coap_uri_t *destParts =
      coap_new_uri(reinterpret_cast<const uint8_t *>(destination.c_str()),
                   destination.length());
  ...
  if (!sess) {
    sess = coap_new_client_session(context, &localAddress, &dstAddr,
                                   COAP_PROTO_UDP);
    coap_session_set_max_retransmit(sess, 10);
    coap_session_init_token(sess, 8,
                            reinterpret_cast<const uint8_t *>("0000000"));
  }
...
void CoapEndpoint::waitForDeath() { loop.join(); }