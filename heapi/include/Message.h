...
/**
 * @brief Represents an application-level message that can transit through the
 * references.
 */
class Message {
private:
...
public:
   ...
   */
  void fillResponse(coap_resource_t *resource, coap_session_t *sess,
                    coap_pdu_t const *request, coap_pdu_t *response);

  /**
   * @brief Converts the Message object to a coap_pdu_t structure.
   *
   * @return coap_pdu_t the structure that will eventually be sent through the
   * references.
   */
