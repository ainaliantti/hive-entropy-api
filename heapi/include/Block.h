...
private:
  /**
   * @brief The communicating node responsible for the computation of this
   * block.
   */
  std::shared_ptr<Peer> responsible;

  /**
   * @brief The time at which the block was created, used to check if it
   * expired.
   */
...
public:
  ...
  /**
   * @brief Constructs a block object using its responsible and bound
   * coordinates in the broader result.
   *
   * @param _responsible The communicating node responsible for the computation
   * of this block.
   ...
   */
  Block(Peer const &_responsible, int const &_startRow, int const &_endRow,
        int const &_startCol, int const &_endCol);

  /**
   * @brief Construct a block object using only the starting coordinates in the
   * broader result.
   *
   * Usually utilized on reception of a message containing insertion
   * coordinates, to compare them to an existing block with more details.
   *
   ...
   *
   * @return std::chrono::steady_clock::time_point The time at which the block
   * was created, used to check if it expired.
   */
  ...
  /**
   * @brief Compares two blocks based on their starting coordinates in the
   * broader result.
   *
   * Specifically used to associate a result packet with the associated pending
   * blocks using insertion points.
   *
