 ...
 */
template <class C> class GlobalContext {
public:
  /**
...
std::map<std::string, C>
    GlobalContext<C>::repository = std::map<std::string, C>();

template <class C> void GlobalContext<C>::registerObject(std::string id, C o) {
  GlobalContext::repository.insert(std::pair<std::string, C>(id, o));
}

template <class C> C &GlobalContext<C>::get(std::string id) {
  try {
...
template <class C> void GlobalContext<C>::unregisterObject(std::string id) {
  repository.erase(id);
}
