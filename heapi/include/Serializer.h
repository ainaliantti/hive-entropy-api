...
public:
   ...
   */
  template <typename T>
  static std::vector<Matrix<T>>
  unserializeMatrices(std::string coded, std::string encoding = "same");
   ...
   * @param encoding The optional encoding type of the elements for the
   * transfer.
   * @return std::string The serialized matrices as a string.
   */
  template <typename T>
  static std::string serialize(std::vector<Matrix<T>> list,
                               std::string encoding = "same");
   ...
   */
  template <typename T>
  static std::string serialize(Row<T> row, Column<T> col,
                               std::string encoding = "same");
   ...
   */
  template <typename T>
  static std::pair<Row<T>, Column<T>>
  unserializeRowColumn(std::string coded, std::string encoding = "same");
   ...
   */
  template <typename T>
  static Matrix<T> unserializeMatrix(std::string const &coded,
                                     std::string const &encoding = "same");
  ...
  static void printSerializedDimensionsBits(size_t const size,
                                            void const *const ptr);
};

template <typename T>
std::vector<Matrix<T>> Serializer::unserializeMatrices(std::string coded,
                                                       std::string encoding) {
  ...
  while (offset < coded.length()) {
    ...
    spdlog::debug("Found matrix of dimensions {}x{} to unserialize", rows,
                  cols);

    std::string current_substring(
        coded.substr(offset, rows * cols * sizeof(T) + 2 * sizeof(uint16_t)));
...
template <typename T>
std::string Serializer::serialize(std::vector<Matrix<T>> list,
                                  std::string encoding) {
  std::string concat = "";
  for (Matrix<T> m : list) {
    concat += serialize(m);
...
template <typename T>
std::string Serializer::serialize(Row<T> row, Column<T> col,
                                  std::string encoding) {
  ...
  if (!std::is_same<T, int>::value && !std::is_same<T, float>::value &&
    ...
    throw "Matrix serialization doesn't support matrices of " +
...
template <typename T>
std::pair<Row<T>, Column<T>>
Serializer::unserializeRowColumn(std::string coded, std::string encoding) {
  if (coded.length() < 1)
  ...
  if (coded.length() - 2 < rowsize * sizeof(T))
    throw "There are not enough coded elements to describe the whole row of "
          "size " +
        rowsize;
  ...
  if (coded.length() - 2 < colsize * sizeof(T))
    throw "There are not enough coded elements to describe the whole column of "
          "size " +
        rowsize;
...
template <typename T>
Matrix<T> Serializer::unserializeMatrix(std::string const &coded,
  ...
  if (coded.length() - 2 * sizeof(uint16_t) != rows * cols * sizeof(T)) {
    ...
    spdlog::error("The serialized size doesn't correspond to the dimensions "
                  "(received {}, needed {}x{}={})",
                  coded.length() - 2 * sizeof(uint16_t), rows, cols,
                  rows * cols * sizeof(T));
    throw "The body of the serialized matrix must be equal to its dimensions "
          "mutliplied together (" +
