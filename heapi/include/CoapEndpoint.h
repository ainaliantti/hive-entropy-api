...
private:
  ...
  /**
   * @brief The local URI oh the address, in the format of
   * "coap://address:port/".
   */
  ...
  /**
   * @brief The mutex used to make modifications of the context object
   * thread-safe.
   *
   */
...
public:
  ...
  /**
   * @brief Adds a resource handling callback method for a combo of a resource
   * path and HTTP-like method.
   *
   ...
   */
  void addResourceHandler(std::string const &path, coap_request_t const &method,
  ...
  /**
   * @brief Deregisters the current handler callback for the specified resource
   * path and HTTP-like method.
   *
   * @param path The Resource path to deregister.
   * @param method The HTTP-like method for which the resource will be
   * deregistered.
   */
  void unregisterResourceHandler(std::string const &path,
                                 coap_request_t const &method);

  /**
   * @brief Waits for the network thread to die before returning.
   *
   * Used in the destructor to not leave a trailing reference to this, and is
   * called after passing keepAlive to false.
   *
   */
  void waitForDeath();
};
